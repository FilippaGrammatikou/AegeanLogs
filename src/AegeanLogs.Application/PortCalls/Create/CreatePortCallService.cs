...
public sealed class CreatePortCallService
{
    ...
    public CreatePortCallService(IAegeanLogsDbContext dbContext, ITransactionManager transactionManager, IValidator<CreatePortCallRequest> validator)
    {
    ...
    public async Task<CreatePortCallResponse> CreateAsync(CreatePortCallRequest request, CancellationToken cancellationToken = default)
    {
        ...
        {
            var errorMessage = string.Join(" ", validationResult.Errors.Select(errorMessage => errorMessage.ErrorMessage));
